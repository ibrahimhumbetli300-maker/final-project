...
const BestSellers = () => {
  ...
  // Səbətə keçid funksiyası
  const handleAddToCart = (product) => {
    // Burada gələcəkdə məhsulu səbətə (state/context) əlavə etmək üçün kod yazacaqsınız
    console.log("Məhsul əlavə edildi:", product);
    
    // Basket səhifəsinə yönləndirmə
    navigate("/basket"); 
  };
        ...
        {products.map((product) => (
          <div key={product.id} className="group cursor-pointer">
            ...
            <div className="space-y-1">
              ...
              {/* 3. Buttona onClick hadisəsi əlavə etdik */}
              <button 
                onClick={() => handleAddToCart(product)}
                className="mt-2 bg-blue-900 text-white px-4 py-2 rounded hover:bg-blue-700 transition-colors"
              >
                Add to Cart
              </button>
            </div>
